...
- **Health Center V2** — node, storage, ZFS, backup, cluster/HA and disk/SMART health with a "why am I seeing this?" detail, configurable thresholds and client-side filters
...
- Optional `health.settings` configures the Health Center thresholds
  (storage usage, backup age in days, swap usage, disk temperature and
  SSD/NVMe remaining life); missing groups fall back to the built-in
  defaults. Values can also be changed from **Settings → Monitoring**.
